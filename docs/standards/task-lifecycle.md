...
| 长期 | 持续存在、无单一终态，随项目演进持续生效 | API 接口体系建设（REQ-046）、数据库总纲维护（REQ-047）、版本末期文档梳理（REQ-050）、经验收件箱机制（REQ-062）等维护/建设类；纯流程义务不占长期行——由 standards/ 承载（判例：REQ-002/010 于 2026-09-22 并入标准关闭） |

- 归属原则：支线在任务描述中体现挂靠的主线；一个任务只属于一个类别；类别随任务形态变化
  调整（如支线成长为独立大类时升级为主线）。
- **主线轮次**：主线由轮次承载（ID 顺序编号，如 ARCH-018），每轮聚焦一个大类目标并聚合
  其支线；支线行在证据列或任务描述中标注归属主线 ID。轮次登记按**一任务一行一壳**（ADR 0016）：
  主线行「任务」列直接链接该轮滚动计划壳，不再另设 PLAN 镜像行；**新增轮次须用户确认后登记**，
  新的小任务原则上并入已有轮次（在对应壳开登记表条目或在 todo 挂支线行），不随意另立新轮次。
...
| 列 | 允许值 | 说明 |
| --- | --- | --- |
| ID | `TASK_ID`（正则 `[A-Z][A-Z0-9]*-[A-Z0-9]{3}(?:-[A-Z][A-Z0-9]*)?`） | 稳定唯一标识，不复用；前缀自然表达任务类型（ARCH=主线实现、REQ=请求、PLAN=计划、DEFECT=缺陷）。**ADR 0016 起 `PLAN` 前缀不再新发用于任务行**（一任务一行一壳，任务行直接链接计划壳；存量 PLAN 行保留至自身关闭）。后缀可选，用于同一父任务的子项分组（如 REQ-067-B10、REQ-070-PARS）；建议后缀不超过 6 字符，优先使用简短语义词（PARS=解析管理、LEARN=学习功能、BATCH=批量子项） |
| 类别 | 主线 / 支线 / 长期 | 见「任务层级」节 |
| 优先级 | 高 / 中 / 低 | 见下方规则 |
| 状态 | 见「状态导航」节 todo 状态枚举 | 与计划状态机联动 |
| 任务 | 文本或指向 `docs/plans/` 的链接 | 需要计划的任务在任务行直接链接计划壳（ADR 0016，不设独立镜像行）；链接后可用「——」追加**一句话任务定义 + 目标/效果**（描述不含嵌套链接与竖线）。todo 行只写「做什么、目标/效果」与关闭条件/下一条件指针；进度、范围边界与工作项细节一律由 plans/ 壳承载 |
...
- todo 使用稳定 ID 保存全部未关闭任务（类型清单见[任务台账](../trackers/todo.md)规则节）。
  候选任务具备范围、前置条件、验证和成功标准后升级为 Plan 并链接 Accepted 计划（一任务对应
  一份计划、只占一个任务行，ADR 0016）；缺陷与关联计划可使用不同 ID 互相引用。
...
- 计划正文是状态事实源；`trackers/todo.md` 镜像全部活跃计划的标题、链接和状态——每个活跃计划
  由**一个任务行**镜像（任务行直接链接壳，ADR 0016，不再另设 PLAN 镜像行）。
