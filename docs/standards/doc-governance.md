...
| `design/` | **固定设计文档**，按模块划分：知识探索、课程学习、课后练习、控制台、仪表盘、文档下载管理（进行中）、文档解析管理等。**只有 plans/ 下评审确认完毕才晋升**为 design/ 文档；此前只在 plans/ 列计划与讨论。 |
...
| `guides/` | 人类可读的**操作文档**与**开发文档**（介绍项目怎么开发、怎么运维），可包含架构信息；为人类设计，由人类判断何时整理，agent 开发时不主动涉及。 |
| `plans/` | **临时计划**：进行中任务的讨论与计划，todo 任务结束即归档。**规范文档**：整理完成后晋升、替代或合并进 `architecture/`、`standards/`、`design/` 固定文档。例外：长期滚动文档（当前仅 [AI Agent 知识收件箱](../plans/ai-agent-knowledge-inbox.md)）常驻本目录，豁免清单由 `test_plan_governance.py::STANDING_DOCS` 守护。 |
...
（见根 `.gitignore`）。其目录结构与管理规则见 [dataset 约束](../design/dataset-conventions.md)。
...
2. **创建/更新目标文档**：
   - 迁入 `design/` 时：以稳定功能名命名文件（如 `course-acquisition-flow.md`，不保留 plan 的
