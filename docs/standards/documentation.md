...
| `architecture/` | **固定架构文档**：总体架构（三项目四服务，交互拓扑由此确定）、每个服务一份服务架构文档、固定 API 接口文档、数据库设计文档（总纲）与 code-map.md。只放确定文档，实时状态不落本目录。 |
| `design/` | **相对确定的设计文档（仅收已评审确定的设计）**：服务契约、dataset 约定、统一配置接口、失败语义和质量门槛；相关任务完成后按三态梳理（合并进确定文档 / 归档 / 保持原状）。未定稿/待评审设计一律随计划在 `plans/` 承载，用户确定后以稳定名称迁入本目录或合并进固定文档（[ADR 0011](../adr/0011-pending-design-location.md)）。 |
...
- `architecture/` 中活跃文档不绑定产品版本号（见下文命名规则）；版本差异由 history/ 归档与
  ADR 决策阶段体现。
- 每次主线 TODO 任务完成后梳理 `guides/` 与 `trackers/`：操作/开发文档随功能变化更新，完成
  的任务移入 completed.md。
...
- `design/` 设计文档三态梳理：相关任务关闭时，设计内容已并入 architecture/ 固定文档的标
  Superseded 并删除（内容由固定文档承接）；已完成使命的存档文档移入 `history/`（如
  `history/baselines/`）；仍具契约价值且任务未完成或后续轮继续使用的保持原状并更新实现状态。
