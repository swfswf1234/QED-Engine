...
- **四服务卡**（2026-08-17 用户裁决落地）：
  - `config`（8900）：状态恒在线；仅**重启**按钮，经 `/self-restart` 端点，确认后约
    3s 自动刷新页面。
  - `tracker`（8901）/ `axiom`（8902）：在线→停止+重启、离线→启动，既有 /services
    语义不变。
  - `web`（8903）：在线→重启、离线→启动，**不提供停止**（避免自掘断界面）；后端
    注册表已含 web 单元，前端以真实状态为准（8900 离线时本地判定兜底 + 去重）。
