...
| 榜单 | 内容 | 用途 | 状态 |
| --- | --- | --- | --- |
| 找资料权威性榜单 | 各教材/版本在数学社区的权威性排序（经典程度、是否名校指定） | 第一轮评估选书参考 | 已登记根仓库 todo（待开始） |
| 找书找得率榜单 | 各来源渠道的命中率/下载成功率实测统计 | QED-Tracker 来源评估矩阵输入（见 QED-018） | 已登记根仓库 todo（待开始） |

榜单为数据收集任务，产出回填本流程阶段 1 的选书规则与 QED-Tracker
`source-discovery.md` 来源评估矩阵，不单独建界面。
