...
关联文档：[2026-08-27-download-ux-flow.md](../plans/2026-08-27-download-ux-flow.md)（PLAN-023 全流程交互规范**主文档**，冲突时以其为准）、[api-contracts](../architecture/api-contracts.md)（5 端点契约）
...
六值与 `shared_tables.py` 的 `STAGE_*` 常量一一对应。流转：
...
| 操作 | 起始状态 | 目标状态 | 前端调用 | 落库写点 |
| --- | --- | --- | --- | --- |
| 探索领域知识 | 未开始 / 待确认（重探） | 已生成（名称确认时）/ 探索中 | `POST /domains/{id}/explore-knowledge` | 会话 ready → 已生成 |
| 导入领域知识 | 未开始 | 已生成 | 文件选择器 → `POST /domains/import` | `import_domain_manual` |
| 确认领域 | 已生成 | 探索中 | `POST /domains/{id}/confirm-domain` | `confirm_domain_info` 写 RUNNING |
| 后台完成 | 探索中 | 待确认 | 5s 轮询感知 | 会话 ready 写 PENDING + `explore_pending={kind:'review_results'}` |
| 确认课程 | 待确认 | 已完成 | `POST /domains/{id}/confirm-knowledge` | apply 成功写 COMPLETED |
| 导入课程提交确认 | 已生成（import_pending） | 已完成 | ConfirmModal 链 | `commit_import_courses` 写 COMPLETED |
...
PLAN-025 §B8 作废、前端直写作废（§2.3）。

### 2.3 前端禁写原则

前端**禁止**直写 `exploration_stage`，一律由 8900 写点驱动；`PATCH /domains` 仅允许更新
描述/学科知识/课程方向等维护字段。课程层 3 态（未开始/探索中/已完成）：探索中→已完成由
`dry_run_course_explore` 会话 ready 感知。
...
- `api/domain_explore.py` = **五态门面**（5 端点，委托 `ExploreSessionManager`）；
- `api/explore.py` = 课程探索通用通道（explore-sessions 会话 CRUD）；
- 两栈共用同一 Manager 实例与写点。

5 端点：`POST /domains/{id}/explore-knowledge`（202）、`POST /domains/{id}/confirm-domain`、
`POST /domains/{id}/confirm-knowledge`、`GET /domains/{id}/explore-status`、`POST /domains/import`（已有）。
...
| --- | --- |
| 未开始 | 领域创建默认；失败重试入口 |
| 已生成 | `_finish_ready`（会话 ready）+ `import_domain_manual`（导入） |
| 探索中 | `confirm_domain_info` 写 RUNNING |
| 待确认 | 会话 ready 写 PENDING + `explore_pending={kind:'review_results'}`；`_apply_domain` 写 PENDING |
| 已完成 | `confirm_course_knowledge` → apply；`commit_import_courses` 写 COMPLETED |
| 失败 | `_run_pipeline` 异常分支写 FAILED + `explore_pending={kind:'failed', error}` |
...
+ `explore_pending` 留存（import_courses 分支同路径）。

### 4.3 explore-status 口径

`active_session` 为**库面推断**（`stage == 探索中` 且 `_active_by_domain` 有登记）；8900 重启后
内存会话丢失 → `task_id=null`，库态直读永远可用，前端据 `active_session=false` 提示「探索会话已失效，
请重试」，不做自动回写。8901 离线兜底：跳过 task 查询仅返回库态。

### 4.4 降级链路规则

| 场景 | 8900 行为 |
| --- | --- |
| 8901 离线 + 导入领域 | `import_domain_manual` 本地 GENERATED + explore_pending={kind:'import_courses'} |
| 8901 离线 + 确认课程 | 409 降级本地 COMPLETED + 待同步 |
| 8901 离线 + explore-status | 库态直读兜底 |
| dry-run 任务失败 | FAILED + explore_pending{kind:'failed', error} |
| 8900 重启（会话丢失） | 不自动回写；active_session=false 承接 |

### 4.5 QED-Tracker 移交清单（REQ-068）

① 书籍状态机 8 路由（decide/start/fail/retry/complete/verify/reject/supersede）未实现 →
书目操作 UI 上游 404（删除教程/修改教程信息同属此缺口）；② `PATCH /courses` 不支持
`exploration_stage`；③ `GET /courses/{domain_id}` 单领域清单未透传；④ `explore_pending.kind`
双形态归一。详见 todo.md REQ-068 行。
...
| 管线失败 | `explore_pending.kind='failed'` → DomainCard 红色提示条 + 失败原因 + 重试 |
...
> （QED-050-D）：`candidate → decided → downloading → downloaded → verified` 终态，
> `failed` 可重试，`rejected`/`superseded` 留痕退出。当前聚焦数学课程体系（13 门），
> 后续轮次扩展计算机科学（AI 方向），五阶段规则通用。
...
- **人工审核**：打开本地下载目录/预览 PDF，确认版本（书名、语言、版次、内容完整性）；
  验收通过（`verify` → verified 终态）或否定（`reject`，硬删 + 留痕）。
