...
| REQ-054 | 支线 | 实现 | 高 | 已完成 | 8900 透传适配探索端点（计划 [exploration-api](../plans/2026-08-arch019-exploration-api.md) §工作项3）：tracker.py 新增透传路由 + tracker_client 扩展 + 契约测试；可与 REQ-055 并行开发（mock 先行） | **2026-08-24 实现完成**：tracker_client 8 方法（start_course_explore/get_explore_run/adopt_explore_run/discard_explore_run/list_explore_runs/start_curriculum_explore/get_curriculum_run/apply_curriculum_run）+ api/tracker 8 路由（§1~7 + 202 状态码/409 结构化 detail 透传）+ 20 契约测试（10 client + 10 route）；门禁：pytest 320 passed + 真实冒烟：8900→8901 课程探索返回 COURSE_NOT_FOUND（结构化 detail 保留）、领域探索返回404（8901 未实现）、LLM qwen 真实调用 ok |
| REQ-055 | 支线 | 请求 | 高 | 已完成 | **请求：QED-Tracker**——课程层探索端点组：`POST /courses/{id}/explore`、`GET /explore-runs/{id}`、`adopt/discard`、历史列表 + 探索记录表（qt_* 私有，表设计归对方）+ draft qt_knowledge 创建；契约见 [exploration-api 计划](../plans/2026-08-arch019-exploration-api.md) §0~5（**已冻结**）；**附带统一数据根适配**（config `_ENV_MAP` 补 `QED_DATA_ROOT→data_root` + 共享布局落盘，源自 data-foundation 计划 §5）；**已传递对方仓库**：承接设计文档 `docs/plans/2026-08-exploration-api-adoption.md` + todo QED-040 已建 | **2026-08-24 回执关闭**：对方提交 `4098a03`（数据根适配 Phase 1）+ `e4072ef`（8 端点 + 双 advisor + 任务编排）；B 组联调真实冒烟通过——课程层 explore→ready（约10s，qwen-plus 3 套推荐 Rudin/Zorich/Tao）→adopt→qt_knowledge 3 行 draft 落库，前端经 8900 可见 |
| REQ-056 | 支线 | 请求 | 中 | 已完成 | **请求：QED-Tracker**——新建领域探索端点组（2026-08-23 裁决读法 1：领域名+探索文档→提议课程体系→apply 写 qed_domain+qed_course）：`POST /curriculum-explore`、curriculum-runs 详情/apply（conflict 拒绝不覆盖）+ 领域/课程手工 CRUD 五端点（手工加课保留）；契约见 [exploration-api 计划](../plans/2026-08-arch019-exploration-api.md) §6~8（**已冻结**）；**已传递对方仓库**：同一承接设计文档 + todo QED-041 已建 | **2026-08-24 回执关闭**：对方提交 `e4072ef` 含领域层 3 端点 + CurriculumExploreAdvisor + 幂等 create_domain/create_course；真实冒烟通过——「高等数学」doc 模式探索 ready（约17s，1 次 qwen 调用 1088 tokens）提议 7 变更（create_domain×1 + create_course×6），全选 apply 零冲突落库（qed_domain 1 / qed_course 6，运行态 applied） |
...
| PLAN-021 | 支线 | Plan | 高 | Accepted | [课程探索 API 契约（ARCH-019·正式文档格式草案）](../plans/2026-08-arch019-exploration-api.md) | 承载 REQ-055/056；**契约已于 2026-08-23 冻结**（用户裁决 A~F 并入，无【待定】残留），确定后并入 api-contracts.md |
...
| REQ-059 | 支线 | 请求 | 高 | 进行中 | **请求：QED-Tracker**——领域课程体系手工维护与重探语义增补（2026-08-24 用户裁决，下载管理左树 v2 全弹窗流配套）：① GET /domains（只读）；② PATCH /domains/{id}（description/stages，name 锁死）；③ 重探语义——apply 对已存在领域的 create_domain 条目跳过标记 skipped（取代 DOMAIN_NAME_CONFLICT），create_course 挂靠既有领域；④ 字段口径：domain_id/course_id 服务端生成、PATCH courses 仅 stage/sort_order/note。**已传递对方仓库**：adoption 计划「REQ-059 增补」章 + api-design 详规 §11 + QED-041 注记（test_documentation 8 passed）。**根仓库侧已完成**：前端 v2 改造（真实体系树/右键菜单/ExploreFlowModal/ErrorBoundary；vitest 148 passed + tsc 零错 + build 成功）+ 8900 透传路由上线（GET /courses 实通 + §8 端点上游 404/405 归一结构化 404 UPSTREAM_NOT_IMPLEMENTED 降级——含补注册遗漏的 POST /domains（曾致添加领域 405）；pytest 340 passed + live 冒烟通过；领域表单按用户裁决去 stages 仅采集名称+描述）；api-contracts.md 已补端点表 | 对方评审接受并实现后回执（提交号 + 测试输出 + skipped 输出结构备案）；前端随回执适配 skipped 展示与手工维护联调验收，回执后关闭 |
...
| ID | 类别 | 类型 | 优先级 | 状态 | 任务 | 证据/下一条件 |
| --- | --- | --- | --- | --- | --- | --- |
| REQ-060 | 支线 | 请求 | 高 | 进行中 | **请求：QED-Tracker 侧提出**——prompt 优化模块配套改造（来源：对方设计 `docs/plans/2026-08-prompt-optimization.md`（Accepted）与 todo QED-043，2026-08-24 用户裁判）：① `qed_llm_calls` 扩展可空列 `task VARCHAR(64)` / `step VARCHAR(32)` / `review_status VARCHAR(16) DEFAULT 'unreviewed'` / `review_note VARCHAR(1000) DEFAULT ''`（`backend/qed_engine/services/llm/call_log.py` 建表与插入同步）；② `GET /api/v1/llm/calls` 增加 task/step/prompt_template/review_status 过滤；③ 新增 `PATCH /api/v1/llm/calls/{id}/review` 审核标注端点；④ web-ui 调用检索页（LlmCalls.tsx）模板聚类展示 + 逐条审核 UI + 新过滤；⑤ 文档登记（database-design.md / llm-gateway-and-model-management.md） | **根仓库实现完成（2026-08-24）**：后端 schema +4 列（call_log.py INSERT/CREATE TABLE + record_call + search_calls + review_call）+ PATCH /review 端点（control.py）+ schemas.py（CallLogItem/ReviewCallRequest/ReviewCallResponse）；前端 LlmCalls.tsx 筛选栏 +4 控件 + 模板列 Tag + 展开行审核区（状态 Tag + 标记审核 Modal）+ api/llm.ts reviewCall + stores/llmCalls.ts reviewItem；pytest 348 passed + vitest 149 passed + tsc 零错 + build 成功；live 冒烟：GET /llm/calls 200 + PATCH /review 404 结构化 + 新过滤参数 200；待回执对方 |
| REQ-061 | 支线 | 请求 | 高 | 待开始 | **请求：QED-Engine 网关缺陷修复（2026-08-24 QED-043 真实评估发现，与 REQ-060 不同源）**：① `services/llm/clients.py` `DEFAULT_TIMEOUT=60.0` 硬编码且 `gateway.call_text` 未向 provider_text_chat/lmstudio_chat 透传超时——qwen3.8-2.4t-a95b 生成长 JSON（课程清单 4000+ 字符）实测 >60s 必现 ReadTimeout（证据：qed_llm_calls call_id 62/64~65/68~69/71~72，duration_ms≈60100、error=模型调用失败 ReadTimeout）；② `gateway.call_text` 收了 `max_tokens` 形参但函数体未使用、provider_text_chat 无 max_tokens 入参——调用方 max_tokens 静默失效。请求：超时可配置（env 或参数，建议默认 ≥300s 或按 endpoint 区分）+ max_tokens 透传上游 | 由根仓库实现；回执 = 提交号 + 长生成复测输出（QED-Tracker 将用 courses 步骤真实 prompt 复测网关链路）；关联 todo QED-043 Phase B0 |
