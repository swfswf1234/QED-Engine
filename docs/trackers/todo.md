...
| ARCH-005 | Plan | 高 | Accepted | [2026-08 前端展示重构（display-redesign-v5）](../plans/2026-08-8903-display-redesign.md) | 计划状态 Accepted；2026-08-06 用户确认（入口页零后台痕迹+使用手册、仪表盘流水线+健康面板、知识体系四级树+（N本）计数、按钮弹层筛选器）；**五期完成（127 passed + ruff clean + JS 语法通过 + 8903 token 实测）**：入口页移除服务状态卡、help-modal 手册、流水线+健康面板、知识体系四级树（领域自适应）、弹层筛选器；待用户浏览器验收后归档 |
