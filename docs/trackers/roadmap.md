...
| **探索（文档切分与召回）** | 替换原始文档对照的管理界面：块级切分校验 → 对话式召回验证（BM25 先行 → 向量库混合检索）→ 知识单元产出，为学习中心问答铺底座；方向设计见 [exploration.md](../design/exploration.md)（2026-08-18 登记，文档解析管理闭环后启动） | 文档解析管理轮 |
| Axiom-Flow 对齐 | 端口 8902、数据目录指向根 dataset/、直读 QED_ 变量、前端迁移 | REQ-003 |
