...
依据 ADR：`../adr/v0.1/0006-engineering-governance-contract.md`
关联测试：`tests/contract/test_test_suite_governance.py`

> **迁移说明（2026-08-30）**：本规范内容已并入 [测试架构与门禁](../../standards/testing.md)
> （测试分层、隔离与门禁 + 治理契约守护面清单/契约头/编写约定/新增流程），解决两份标准 4 处
> 内容重叠与「互补不重叠」声明模糊问题。本文保留作历史审计留档，正文不再维护；如需修改治理
> 契约规则，直接修改 standards/testing.md。
...
门禁由[测试架构与门禁](../../standards/testing.md)规定；标准/ADR/计划等文档的元数据规则在其对应标准正文。
本规范与 testing.md 互补不重叠：testing.md 管「全部测试的架构与门禁」（分层职责、隔离替身、
覆盖率）；本文管「治理契约测试的守护面清单、契约头、编写约定与新增流程」——即 contract 层的
编写规范。守护面清单是唯一维护位置，`test_test_suite_governance.py` 从本文提取清单常量源。
...
1. 治理规则变化：按[ADR 治理规范](../../standards/adr-governance.md)判断是否需新增 ADR，修改对应标准正文。
...
子项目对齐请求以根仓库 todo 登记（见[跨项目协作规范](../../standards/cross-project-collaboration.md)），
在其仓库内实现并遵守其门禁。
