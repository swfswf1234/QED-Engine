...
**目标**：定义「文档解析管理」前端（8903 `#/admin/parsing`）的可实现级设计——左侧
领域→课程→书目树、右侧原页 vs 解析结果对照与人工校验修改，取代现有「左树右对照」过渡形态
（`BlockView` + `af_block_reviews` 旧模型）。

**成功标准**：

1. 左侧树按 领域→课程→书目 三级展示，书目节点含解析进度（`pages_done/page_count`）与状态点；
2. 未解析书目点开后自动 ingest 页图，展示原页图 + 「未解析」空态 + 解析入口；
3. 右侧对比区：原页图叠加语义 bbox 方框，与右侧块列表双向联动选中；
4. 支持块级 判定（一致/不一致）+ 备注 + 文字修正 + bbox 拖拽/缩放修改，保存后可回显；
5. 工具栏「刷新」为唯一同步入口（同步书目 + 重载树/页），不单设「同步」按钮；
6. `vitest` + `tsc` + `npm run build` 全绿，浏览器手动验收通过。

## 范围与非目标

**范围**：8903 解析管理页——`web-ui/src/pages/Parsing.tsx`、`web-ui/src/stores/parsing.ts`、
`web-ui/src/api/axiom.ts` 及新增 `web-ui/src/components/parsing/` 组件。

**非目标**：

- 8900 / 8902 侧实现（见[与 Axiom-Flow 交互全链路](../../../plans/2026-09-14-parsing-management-axiom-flow-chain.md)）；
- 切分与对话式召回（探索轮，见 [document-chunking-recall.md](../../../design/document-chunking-recall.md)）；
- 整书级质量报告、批量验收 UI（后续轮）；
...
- 页面容器沿用现有外层：`Layout.Content`（`padding 24`、`maxWidth 1600`、`width 96%`、
  居中）+ 顶部标题行（`文档解析管理` + 右侧工具栏）；
...
| `web-ui/src/components/parsing/CompareView.tsx` | 右侧容器：书目标题、页数据编排、态分流（未 ingest/未解析/已解析/失败） | 新增 |
...
（`tryRenderKatex` / `renderInlineMath` / `blockText` / `TYPE_LABELS`）；`BlockView.tsx` 退役
（判定迁入 `BlockEditor`，渲染迁入 `BlockList`）。

### 3. 状态模型（stores/parsing.ts 重构）
...
- `BookMeta` 更新：`ingest_status`（`none|ingesting|ingested|failed`）、`parse_status`
  ...
  `display_title`；移除旧 `author`/`strategy` 依赖（兼容保留可选）。
...
| 场景 | 判定 | 表现 |
| --- | --- | --- |
| 8900 不可达 | 请求失败（非 http 503 上游语义） | 顶部错误横幅 + 「刷新」重试；树/页显示空态 |
...
- 恢复 `Parsing.tsx` / `stores/parsing.ts` / `api/axiom.ts` 到重构前版本，删除新增
  `components/parsing/`；`BlockView.tsx` 从 Git 锚点恢复；
- 影响范围仅解析管理页，不影响其他界面。
