...
关联 Tracker：docs/trackers/todo.md（PLAN-031）
...
成功标准：
1. 服务在线 + 三行图表（领域/课程/书籍）+ 统计数字结构完整记录
...
- 标题：`领域名 · 探索进度 X/Y`（Y=课程总数，X=已完成课程数）
- 聚合函数：`buildDomainProgress(courseSystem, details)`

### 区域 3：课程表（第二行，超过 3 个换行）

- 每个领域一张饼图卡片，无课程的领域不展示
...
- 标题：`领域名 · 课程进度`
- 聚合函数：`buildCourseProgress(courseSystem, details)`

### 区域 4：文档下载进度（第三行，超过 3 个换行）
...
- 标题：`领域名 · 文档下载进度`
- 聚合函数：`buildBookDownloadProgress(courseSystem, details)`
...
State:
  ...
  courseSystem: DomainSystem[]             // 领域课程体系（/courses，新增）
...
| 函数 | 输入 | 输出 | 用途 |
| --- | --- | --- | --- |
| `buildDomainProgress(courseSystem, details)` | 领域课程体系 + 教程详情 | `DownloadSlice[]` | 领域探索进度饼图（按领域） |
...
| 文件 | 职责 |
| --- | --- |
| `pages/Dashboard.tsx` | 主页面，服务在线 + 三行图表 + 统计数字 |
...
| `stores/index.ts` | 类型定义（DomainSystem/BookRecord 等，需更新 BookRecord） |
...
### BookRecord（需更新，对齐 qt_books 书库化重构 QED-050-D）
...
2. **独立性铁律**：8901 离线 → 三行图表降级，不影响服务在线卡片
...
4. 人工验收：8903 打开 `#/admin/dashboard` 检查服务在线 + 三行图表 + 统计数字
