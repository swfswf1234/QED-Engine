...
关联设计：[前端重构设计](../../../architecture/frontend-architecture.md)、
[配置中心 API 契约](../../../architecture/api-contracts.md)（监控与诊断域登记，实施后置）、[服务控制设计](../../../design/service-hosting.md)
...
- 2026-08-16 用户方向裁决：前端重构为当前最高优先级，**本轮只做前端部分改造**；后端
  ...
  并行承接（设计文档 [backend-domain-split.md](../../../architecture/backend-architecture.md) 深化版）；
...
- 后端三域拆分（[backend-domain-split.md](../../../architecture/backend-architecture.md)）。
