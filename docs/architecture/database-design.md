...
QED-Tracker 侧 `docs/architecture/database-shared-tables.md`（`qed_*`）与
`database-private-tables.md`（`qt_*`）为 qed 库全部表**唯一事实源**；Axiom-Flow 侧
`docs/architecture/database-design.md` 为 `af_*` 四表（af_books / af_parse_jobs / af_pages /
af_block_edits）结构事实源，设计正文见
...
| `af_books` | 私有 | Axiom-Flow | 一册已验证书（qt_books 只读快照 + ingest/解析进度 + 生效版本指针） | **已落地**（Axiom-Flow Alembic 建表维护） | Axiom-Flow database-design.md |
| `af_parse_jobs` | 私有 | Axiom-Flow | 一次解析任务＝一个解析版本（逐页/全本，状态与进度） | **已落地**（同上） | Axiom-Flow database-design.md |
| `af_pages` | 私有 | Axiom-Flow | 一页解析状态与质量信号（按解析版本作用域） | **已落地**（同上） | Axiom-Flow database-design.md |
| `af_block_edits` | 私有 | Axiom-Flow | 一次块级人工编辑（判定/备注/文字/范围，绑定解析版本） | **已落地**（同上） | Axiom-Flow database-design.md |
...
### qed_domain（领域表，共享，QED-031）

一行 = 一个学科领域（当前仅 math，预留扩展）。三项目可读；QED-Tracker 唯一写权限
（Alembic 建表维护）。**DDL 与字段语义唯一事实源：QED-Tracker `shared-tables.md`**，
本文件登记结构供跨项目索引。
...
CREATE TABLE qed_domain (
  ...
  exploration_stage  VARCHAR(20)   NOT NULL DEFAULT '未开始', -- 流程状态（6 态）
  classic_tracks     JSON          NOT NULL,           -- 课程方向 [{name,summary,kind}] 0~4 项
  stages             JSON          NOT NULL,           -- 学习阶段顺序（四档）
...
| 6 | `exploration_stage` | VARCHAR(20) | `"未开始"` | 流程状态枚举（6 态，见下文状态机） |
| 7 | `classic_tracks` | JSON | `[]` | 课程方向，JSON 数组 [{name, summary, kind}]，0~4 项。`kind`：`main`=主干 / `branch`=分支 |
| 8 | `stages` | JSON | —（无默认值） | 学习阶段顺序列表，四档：`["基础","主干","分支","前沿"]` |
| 9 | `path_results` | JSON | `null` | 学习流程，可空。含 notes/edges[{from,to}]/graph_td |
| 10 | `explore_pending` | JSON | `null` | 探索待确认载荷（REQ-067-B12）：`待确认` = `{kind:"review_results", ...}`；`失败` = `{kind:"failed", error:"..."}`；其余状态 NULL |
| 11-14 | audit | — | — | created_by / updated_by / created_at / updated_at |

**exploration_stage 状态机（6 态，REQ-067-B12）**：
...
|---|---|---|---|
| 未开始 | 手动创建 | 创建方 | NULL |
| 已生成 | 领域探索第一轮报告就绪 | **8900**（写权限例外） | NULL |
| 探索中 | 第一轮已确认，第二轮进行中 | **8900**（同上） | NULL |
| 待确认 | 第二轮报告就绪，等待用户采纳 | **8900**（同上） | `{kind:"review_results", stage:"domain", ...}` |
| 已完成 | 第二轮审阅采纳落库 | **QED-Tracker 8901** | NULL（采纳时清空） |
| 失败 | 探索失败 / 服务重启中断 | **QED-Tracker 8901** | `{kind:"failed", error:"..."}` |

写入权限例外（REQ-064④）：8900 离线降级时可直写 description、stages、exploration_stage、explore_pending；level、scope、classic_tracks、path_results 只归 8901 写。

### qed_course（课程表，共享，QED-031）

一行 = 一门课程。三项目可读；QED-Tracker 唯一写权限（Alembic 建表维护）。
**DDL 与字段语义唯一事实源：QED-Tracker `shared-tables.md`**。
...
| 7 | `stage` | VARCHAR(32) | — | 所属学习阶段，值域来自 qed_domain.stages（四档：基础/主干/分支/前沿） |
...
| 11 | `exploration_stage` | VARCHAR(20) | `"未开始"` | 流程状态枚举（6 态，同 qed_domain，见下文） |
| 12 | `explore_pending` | JSON | `null` | 探索待确认载荷：`待确认` = `{kind:"review_results", tutorials:[...]}`；`失败` = `{kind:"failed", error:"..."}`；其余 NULL |
| 13-16 | audit | — | — | created_by / updated_by / created_at / updated_at |

**stage 字段值域**：来自 `qed_domain.stages`（四档：`基础`=入门基石、`主干`=方向主干、`分支`=方向细分/拓展、`前沿`=研究前沿/论文驱动，2026-08-29 用户裁定）。管线 courses@v8 输出的 `tier` 已对齐更名为 `stage`，落库不再需要映射。

**exploration_stage 状态机（6 态，同 qed_domain）**：课程探索为**单步、单轮**——报告生成即进入 `待确认`，确认后 `已完成`。写主体口径同 qed_domain：8900 负责探索过程流转（已生成/探索中/待确认），QED-Tracker 8901 负责验收终态（已完成）与失败清理。
...
[llm-gateway-and-model-management](../design/llm-gateway.md)：
...
- `af_books`：一册已验证书（qt_books 只读快照：book_id/domain_id/course_id/course_name/
  knowledge_id/title/part/display_title/authors/file_path + sha256/ingest_status/
  parse_status/last_parse_result/pages_done + active_job_id 生效解析版本指针）。
...
（根侧 DDL 快照已按 ADR 0017 删除，指针见[文档解析管理·全链路设计](../design/parsing-flow.md) §2）；
四表由 Axiom-Flow Alembic 独立建表与维护。
