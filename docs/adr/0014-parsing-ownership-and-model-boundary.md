...
3. **解析调用不经 8900 网关**：Axiom-Flow 经引擎适配器直连模型服务（如 MinerU 5002），
   模型服务地址可配置（部署形态需要时可指向 8900 提供的代理地址，契约不变）；8900
   `/llm/vision` 降级为控制台测试与通用视觉用途，不再承载解析流量。（勘误：2026-09-16
   MinerU 端口 8002 → 5002，变量名 `QED_MINERU_URL` → `QED_OCR_MODEL_URL`；不影响本决定语义。）
