...
- 用户 2026-08-18 裁决（设计文档 §1）：左树四层（高等数学→分类→课程→教程叶子+进度）、
  右侧流程筛选（搜索/确认/下载/验收）+ 保留原三个下拉、书行卡去 kind、书行排序、
...
2. **右侧筛选栏**：四个下拉并存——领域（选项含 高等数学/分析/代数/概率）、课程、状态、
   流程（全部/搜索/确认/下载/验收），AND 叠加；流程=书行级过滤，无匹配书行的教程行隐藏。
3. **书行卡**：去掉 kind 标签，保留 状态+roles+作者/版本/页数。
4. **书行排序**：中文教材→中文习题集→其余；组内册数递增（第一册=1…上/中/下=1/2/3、
   单册=0 组首、答案册=99 组尾）。
...
- `web-ui/src/pages/Downloads.tsx`：筛选栏四下拉（+流程）、书行卡去 kind、排序应用、过滤逻辑。
...
- 浏览器验收要点：
  ...
  - 筛选栏：四下拉并存；选「验收」仅显示已验收书行、无匹配教程行隐藏；
  - 书行卡：无 kind 标签；套2 内 菲赫 3 卷（第一/二/三册）→ 谢惠民 上下册 排序正确；
