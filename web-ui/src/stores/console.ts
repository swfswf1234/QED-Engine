/**
 * 控制台 store（Phase 2）
 * - 服务快照（/services）+ MySQL（/config/database）+ GPU（/monitor/gpu）+
 *   LM Studio（/monitor/lmstudio）+ MinerU（/monitor/mineru）五路独立拉取，互不拖累
...
import { databaseTest, llmTestText, llmTestVision } from '../api/llm';
import type { DatabaseStatus, GpuStatus, LmStudioStatus, MineruStatus, ServiceStatus } from './index';
...
async function runTest(
  set: (partial: Partial<ConsoleStore>) => void,
  kind: 'db' | 'text' | 'vision',
...
export interface ConsoleStore {
  ...
  dbError: string | null;
  /** GPU + 系统内存总览（顶部总览条；探测失败仅置 gpuError） */
  gpu: GpuStatus | null;
  /** GPU 探测独立错误（不影响服务卡展示） */
...
export const useConsoleStore = create<ConsoleStore>((set, get) => ({
  ...
  fetchAll: async () => {
    ...
    if (operating) return;
    set({ loading: true });
    // 五路并行独立拉取：services 失败 → 整体横幅；database/gpu/lmstudio/mineru 失败 → 仅对应字段降级
    ...
    const [[services, servicesErr], [dbStatus, dbErr], [gpu, gpuErr], [lmstudio, lmstudioErr], [mineru, mineruErr]] =
      await Promise.all([
